...
namespace PSI
{
    ...
    public:
        ...
        std::atomic<uint64_t> mSeedIndex;
...
//#endif
	...
	public:
		...
		SocketGroupList::iterator getSocketGroup(const std::string& name, uint64_t id);

		void stopListening();
		uint64_t mPort;
		boost::asio::ip::tcp::endpoint mAddress;

		void bind(uint32_t port, std::string ip, boost::system::error_code& ec);
		void start();
		void stop();
		bool stopped() const;
