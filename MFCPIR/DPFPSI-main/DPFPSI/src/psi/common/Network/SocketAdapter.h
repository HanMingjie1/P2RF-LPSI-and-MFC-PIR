...
namespace PSI
{
    ...
    public:
        ...
        virtual void async_recv(
            std::span<boost::asio::mutable_buffer> buffers, 
        ...
        // @fn [input]:   A call back that should be called on completion of the IO
        virtual void async_send(std::span<boost::asio::mutable_buffer> buffers, io_completion_handle&& fn) = 0;
    ...
    public:
        ...
        struct Operation
        {
            ...
            std::vector<std::span<uint8_t>> mBuffers;
            Type mType;
...
#error "require move"
  ...
  && !defined(BOOST_ASIO_ENABLE_CANCELIO)
        ...
        void async_recv(std::span<boost::asio::mutable_buffer> buffers, io_completion_handle&& fn) override
        {
            boost::asio::async_read(mSock, buffers, std::forward<io_completion_handle>(fn));
        }

        void async_send(std::span<boost::asio::mutable_buffer> buffers, io_completion_handle&& fn) override
        {
