...
namespace PSI {
	...
	SessionBase::SessionBase(IOService& ios) 
		: mRealRefCount(1)
		, mWorker(ios, "Session:" + std::to_string((uint64_t)this)) 
	{}

	void Session::start(IOService& ioService, std::string remoteIP, uint32_t port, SessionMode type, std::string name)
	{
        TLSContext ctx;
    ...
    void Session::start(IOService& ioService, std::string ip, uint64_t port, SessionMode type, TLSContext& tls, std::string name)
    {
        ...
        mBase->mPort = static_cast<uint32_t>(port);
        mBase->mMode = (type);
        ...
        {
			// PRNG prng(ioService.getRandom(), sizeof(block) + sizeof(uint64_t));
			// mBase->mSessionID = prng.get();
			RAND_bytes((uint8_t*)&mBase->mSessionID,sizeof(mBase->mSessionID));
#ifdef ENABLE_WOLFSSL
	...
	Session::Session(IOService & ioService, std::string remoteIP, uint32_t port, SessionMode type, std::string name)
	{
		start(ioService, remoteIP, port, type, name);
	}

    Session::Session(IOService& ioService, std::string remoteIP, uint32_t port
