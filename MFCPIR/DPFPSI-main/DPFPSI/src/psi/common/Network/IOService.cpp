...
#define IF_LOG(m)
    ...
    void Acceptor::bind(uint32_t port, std::string ip, boost::system::error_code& ec)
    {
        ...
        if (mAddress.port() != port)
            throw std::runtime_error("rt error at " );
    ...
    void Acceptor::sendServerMessage(std::list<details::PendingSocket>::iterator sockIter)
    {
        ...
        sockIter->mSock.async_send(buffer, [this, sockIter](const error_code& ec, uint64_t bytesTransferred) {
    ...
    void Acceptor::recvConnectionString(std::list<details::PendingSocket>::iterator sockIter)
    {
        ...
        sockIter->mBuff.resize(sizeof(uint32_t));
        auto buffer = boost::asio::buffer((char*)sockIter->mBuff.data(), sockIter->mBuff.size());
        sockIter->mSock.async_receive(buffer,
            [sockIter, this](const boost::system::error_code& ec, uint64_t bytesTransferred)
            {
                if (!ec)
                {
                    LOG_MSG("Recv header with socket#" + std::to_string(sockIter->mIdx));

                    auto size = *(uint32_t*)sockIter->mBuff.data();

                    sockIter->mBuff.resize(size);
                    auto buffer = boost::asio::buffer((char*)sockIter->mBuff.data(), sockIter->mBuff.size());

                    sockIter->mSock.async_receive(buffer,
                        bind_executor(mStrand, [sockIter, this](const boost::system::error_code& ec3, uint64_t bytesTransferred2) {
                            if (!ec3)
                            {
    ...
    Acceptor::SocketGroupList::iterator Acceptor::getSocketGroup(const std::string& sessionName, uint64_t sessionID)
    {
        ...
        std::unique_ptr<BoostSocketInterface> s)
    {
        ...
        boost::asio::dispatch(mStrand, [this, name, ss]() {
            ...
            if (sessionGroupIter != mGroups.end())
            {
                ...
                std::stringstream s;
                s << std::hex << (uint64_t) & *sessionGroup->mBase.lock();
    ...
    IOService::IOService(uint64_t numThreads)
        :
        ...
        mWorker(*this, "ios")
    {
        ...
        for (auto& thrdProm : mWorkerThrds)
        {
            ...
            thrd = std::thread([this, i, &prom]()
                {
                    // std::thkread::setThreadName("io_Thrd_" + std::to_string(i));
                    mIoService.run();
                    prom.set_value();
...
#else
                    std::cout << "IOSerive::stop() is waiting for work to finish" << std::endl;
#endif
                }
    ...
    PSI::details::SessionGroup::~SessionGroup()
    {
        if (hasSubscriptions())
