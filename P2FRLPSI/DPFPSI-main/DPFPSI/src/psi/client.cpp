...
namespace PSI
{
    ...
    void PSIClient::InsectionCheck(std::vector<DHOPRF::OPRFValueOpenssL>& oprf_input,const std::vector<Item>& input){
        ...
        // 输出交集结果到文件
        std::ofstream intersection_file("/home/hmj/P2FRLPSI/DPFPSI-main/DPFPSI/intersection_results.txt");
        if (intersection_file.is_open()) {
            intersection_file << "=== PSI Protocol Results ===" << std::endl;
            intersection_file << "Intersection Size: " << Ins.size() << std::endl;
            intersection_file << "=================================" << std::endl;
            
            for(size_t i = 0; i < Ins.size(); i++){
                intersection_file << "Intersection[" << i << "]:" << std::endl;
                intersection_file << "  Item: ";
                auto item_data = Ins[i].first.get_as<uint8_t>();
                for(size_t j = 0; j < item_data.size(); j++){
                    intersection_file << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(item_data[j]);
                }
                intersection_file << std::dec << std::endl;
                
                intersection_file << "  Label: ";
                auto label_data = Ins[i].second.get_as<uint8_t>();
                for(size_t j = 0; j < label_data.size(); j++){
                    intersection_file << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(label_data[j]);
                }
                intersection_file << std::dec << std::endl;
                intersection_file << std::endl;
            }
