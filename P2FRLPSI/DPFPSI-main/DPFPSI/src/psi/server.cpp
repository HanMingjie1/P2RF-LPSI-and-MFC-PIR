...
namespace PSI
{
    namespace Server
    {
        ...
        void PSIServer::DHBasedPSI_start(std::string SelfAddress,std::string AidAddress,const std::vector<Item>& input,const std::vector<PSI::Label>& input_Label){
            ...
            double offline_mb = (static_cast<double>(Mask_byte_size) * static_cast<double>(cuckoo::table_size)) / (1024.0 * 1024.0);
            clocks.setDurationEnd("Offline");
            // std::cout <<__FILE__<<":" << __LINE__ << std::endl;
            clocks.setDurationStart("online");

            runDH(chlsC);
            double online_bytes = 0.0;
            for(auto &chl : chlsC){
                online_bytes += static_cast<double>(chl.getTotalDataSent()+chl.getTotalDataRecv());
            }
            for(auto &chl : chlsA){
                online_bytes += static_cast<double>(chl.getTotalDataSent()+chl.getTotalDataRecv());
            }
            double online_mb = online_bytes/1024.0/1024.0;
            {
                std::ofstream mb_file("/home/hmj/P2FRLPSI/DPFPSI-main/DPFPSI/mb_stats.txt", std::ios::trunc);
                if (mb_file.is_open()) {
                    mb_file << std::fixed << std::setprecision(3) << offline_mb << "\n";
                    mb_file << std::fixed << std::setprecision(3) << online_mb << "\n";
                }
            }
            for(auto &chl : chlsC){ chl.close(); }
            for(auto &chl : chlsA){ chl.close(); }
